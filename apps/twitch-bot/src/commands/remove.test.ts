...
describe('handleRemoveCommand', () => {
  it('sends usage when no username provided', async () => {
    await handleRemoveCommand(makeEvent('!remove') as unknown as ChatMessageEvent);
  ...
  it('confirms removal when user found', async () => {
    ...
    await handleRemoveCommand(makeEvent('!remove TrainerAsh') as unknown as ChatMessageEvent);
