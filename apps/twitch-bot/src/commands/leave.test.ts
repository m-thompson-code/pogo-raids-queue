...
vi.mock('../messages.js', () => ({
  messages: {
    leaveRemoved: (u: string) => `leaveRemoved:${u}`,
    leaveNotInQueue: (u: string) => `leaveNotInQueue:${u}`,
  },
}));
...
describe('handleLeaveCommand', () => {
  it('confirms with pogo username when found', async () => {
    ...
    await handleLeaveCommand(makeEvent() as unknown as ChatMessageEvent);
  ...
  it('falls back to twitch username when not in queue', async () => {
    ...
    await handleLeaveCommand(makeEvent() as unknown as ChatMessageEvent);
    expect(sendChatMessage).toHaveBeenCalledWith('leaveNotInQueue:moo');
  });
